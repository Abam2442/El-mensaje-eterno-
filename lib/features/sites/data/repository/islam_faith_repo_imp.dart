...
class IslamFaithRepositoryImp extends IslamFaithRepository {
  ...
  Future<Either<Failure, List<FixedEntities>>> getArtical() async {
    ...
    try {
      Get.find<Logger>().i("Start `getArtical` in |IslamFaithRepositoryImp|");
      var artical = await islamFaithLocalDataSource.getArtical();
      Get.find<Logger>()
          .w("End `getArtical` in |IslamFaithRepositoryImp| ${artical.length}");
      return Right(artical);
    } catch (e) {
      Get.find<Logger>().e(
          "End `getArtical` in |IslamFaithRepositoryImp| Exception: ${e.runtimeType}");
